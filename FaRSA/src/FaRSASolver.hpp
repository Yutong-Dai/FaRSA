...
class FaRSASolver
{
   public:
     ...
     */
    inline double const time() const
    {
        return (quantities_.endTime() - quantities_.startTime()) / (double)CLOCKS_PER_SEC;
    };
    /**
     * Get time in evaluations
     ...
     */
    inline double const timeFaRSA() const
    {
        return (quantities_.endTime() - quantities_.startTime() - quantities_.evaluationTime()) /
   ...
   private:
    ...
    //@{
    void addOptions();
    void evaluateFunctionstAtCurrentIterate();
    void computeProximalGradientUpdateAtCurrentIterate();
    void printFooter();
    void printHeader();
