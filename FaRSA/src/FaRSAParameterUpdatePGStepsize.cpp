...
{
    options->addStringOption(reporter, "PU_PGStepsize_method", "DecreaseByFraction",
    ...
    options->addDoubleOption(reporter, "PU_PGStepsize_decrease_fatcor", 0.8, 0.0, 1.0,
                             "Factor used to  decrease the proximal gradient stepsize.\n"
                             "Default     : 0.8.");
    options->addDoubleOption(reporter, "PU_PGStepsize_increase_fatcor", 1.1, 1.0, FARSA_DOUBLE_INFINITY,
                             "Factor used to  increase the proximal gradient stepsize.\n"
                             "Default     : 1.1.");
    options->addDoubleOption(reporter, "PU_PGStepsize_upper_bound", 1.0, 0.0, FARSA_DOUBLE_INFINITY,
...
{
    options->valueAsString(reporter, "PU_PGStepsize_method", method_);
    options->valueAsDouble(reporter, "PU_PGStepsize_decrease_fatcor", decrease_factor_);
    options->valueAsDouble(reporter, "PU_PGStepsize_increase_fatcor", increase_factor_);
    options->valueAsDouble(reporter, "PU_PGStepsize_upper_bound", upper_bound_);
}  // end getOptions

// Initialize
void ParameterUpdatePGStepsize::initialize(const Options* options, Quantities* quantities, const Reporter* reporter) {}

void ParameterUpdatePGStepsize::update(const Options* options, Quantities* quantities, const Reporter* reporter,
                                       Strategies* strategies)
{
    ...
    try
    {
        if (method_.compare("DecreaseByFraction") == 0)
        {
            ...
            {
                quantities->setStepsizeProximalGradient(quantities->stepsizeProximalGradient() * decrease_factor_);
            }
        ...
        else if (method_.compare("Heuristic") == 0)
        {
            ...
            {
                quantities->setStepsizeProximalGradient(quantities->stepsizeProximalGradient() * increase_factor_);
            }
            THROW_EXCEPTION(PU_SUCCESS_EXCEPTION, "Parameter Update Successful.");
        }
        else if (method_.compare("LipschitzEstimate") == 0)
        {
            ...
            */
            ...
            double directional_derivative = quantities->currentIterate()->gradientSmooth()->innerProduct(*step_taken);
            double model_decrease = directional_derivative + 0.5 * step_taken_norm_square * Lipschitz_estimate_old;
            double temp = Lipschitz_estimate_old + 2.0 * (actual_decrease - model_decrease) / step_taken_norm_square;
            // safeguard
            if ((actual_decrease - model_decrease) > 0.0)
            {
                Lipschitz_estimate_old = fmax(2.0 * Lipschitz_estimate_old, fmin(1e3 * Lipschitz_estimate_old, temp));
            }
