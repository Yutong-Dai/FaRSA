...
void Quantities::addOptions(Options* options, const Reporter* reporter)
{
    ...
    options->addDoubleOption(
        reporter, "cpu_time_limit", 1e+04, 0.0, FARSA_DOUBLE_INFINITY,
        ...
        "Default     : 1e+04.");
    options->addDoubleOption(
        reporter, "scaling_threshold", 1e+02, 0.0, FARSA_DOUBLE_INFINITY,
    ...
    options->addDoubleOption(
        reporter, "linesearch_armijo_eta", 1e-04, 0.0, 1.0,
        ...
        "Default     : 1e-04.");
    options->addDoubleOption(
        reporter, "linesearch_stepsize_decrease_factor", 0.8, 0.0, 1.0,
        "Constant used to decrease the stepsize during the linesearch.\n"
        "Default     : 0.8.");
    options->addDoubleOption(reporter, "kappa1_max", 1e+06, 0.0,
                             FARSA_DOUBLE_INFINITY,
                             "kappa1_max.\n"
                             "Default     : 1e+06.");
    options->addDoubleOption(reporter, "kappa1_min", 1e-05, 0.0,
                             FARSA_DOUBLE_INFINITY,
                             "kappa1_min.\n"
                             "Default     : 1e-05.");
    options->addDoubleOption(reporter, "kappa2_max", 1e+05, 0.0,
                             FARSA_DOUBLE_INFINITY,
                             "kappa2_max.\n"
                             "Default     : 1e+05.");
    options->addDoubleOption(reporter, "kappa2_min", 1e-06, 0.0,
                             FARSA_DOUBLE_INFINITY,
                             "kappa2_min.\n"
                             "Default     : 1e-06.");
    options->addDoubleOption(reporter, "kappa_increase_factor", 10.0, 1.0,
                             FARSA_DOUBLE_INFINITY,
    ...
    options->addIntegerOption(
        reporter, "function_evaluation_limit", 1e+05, 0, FARSA_INT_INFINITY,
        "Limit on the number of function evaluations performed.\n"
        "Default     : 1e+05.");
    options->addIntegerOption(
        reporter, "gradient_evaluation_limit", 1e+05, 0, FARSA_INT_INFINITY,
        "Limit on the number of gradient evaluations performed.\n"
        "Default     : 1e+05.");

    options->addIntegerOption(
        reporter, "iteration_limit", 1e+04, 0, FARSA_INT_INFINITY,
...
void Quantities::getOptions(const Options* options, const Reporter* reporter)
{
    ...
    options->valueAsDouble(reporter, "iterate_norm_tolerance",
                           iterate_norm_tolerance_);
    options->valueAsDouble(reporter, "stationarity_tolerance",
                           stationarity_tolerance_);
    options->valueAsDouble(reporter, "scaling_threshold", scaling_threshold_);
    options->valueAsDouble(reporter, "linesearch_armijo_eta",
                           linesearch_armijo_eta_);
    options->valueAsDouble(reporter, "linesearch_stepsize_decrease_factor",
    ...
    options->valueAsDouble(reporter, "kappa_increase_factor",
                           kappa_increase_factor_);
    options->valueAsDouble(reporter, "kappa_decrease_factor",
                           kappa_decrease_factor_);

    // Set integer options
    options->valueAsInteger(reporter, "function_evaluation_limit",
                            function_evaluation_limit_);
    options->valueAsInteger(reporter, "gradient_evaluation_limit",
...
// Initialization
bool Quantities::initialize(
    const std::shared_ptr<FunctionSmooth>    function_smooth,
    ...
    const Vector&                            initail_point)
{
    ...
    if (function_smooth->numberOfVariables() !=
        function_nonsmooth->numberOfVariables())
    {
    ...
    // Declare iterate
    std::shared_ptr<Point> initial_iterate(
        new Point(function_smooth, function_nonsmooth, v, 1.0));
...
void Quantities::print(const Reporter* reporter)
{
    ...
    reporter->printf(R_SOLVER, R_BASIC,
                     "cpu time limit (seconds)...................... : %+.4e\n",
                     cpu_time_limit_);
    reporter->printf(R_SOLVER, R_BASIC,
                     "iterate norm tolerance........................ : %+.4e\n",
                     iterate_norm_tolerance_);
    reporter->printf(R_SOLVER, R_BASIC,
                     "stationarity tolerance........................ : %+.4e\n",
                     stationarity_tolerance_);
    reporter->printf(R_SOLVER, R_BASIC,
                     "function evaluation limit_.................... : %d\n",
                     function_evaluation_limit_);
    reporter->printf(R_SOLVER, R_BASIC,
                     "gradient evaluation limit..................... : %d\n",
                     gradient_evaluation_limit_);
    reporter->printf(R_SOLVER, R_BASIC,
                     "iteration limit............................... : %d\n",
                     iteration_limit_);
    reporter->printf(
        ...
        "---------------------- Algorithmic Choices ---------------------\n");
    reporter->printf(R_SOLVER, R_BASIC,
                     "scaling threshold............................. : %+.4e\n",
                     scaling_threshold_);
    reporter->printf(R_SOLVER, R_BASIC,
                     "linesearch armijo eta......................... : %+.4e\n",
                     linesearch_armijo_eta_);
    reporter->printf(R_SOLVER, R_BASIC,
                     "linesearch stepsize decrease factor........... : %+.4e\n",
                     linesearch_stepsize_decrease_factor_);
    reporter->printf(R_SOLVER, R_BASIC,
                     "linesearch max backtrack...................... : %d\n",
                     linesearch_max_backtrack_);
    reporter->printf(R_SOLVER, R_BASIC,
                     "kappa1 max.................................... : %+.4e\n",
                     kappa1_max_);
    ...
    reporter->printf(R_SOLVER, R_BASIC,
                     ...
                     kappa2_min_);
    reporter->printf(R_SOLVER, R_BASIC,
                     "kappa increase factor......................... : %+.4e\n",
                     kappa_increase_factor_);
    reporter->printf(R_SOLVER, R_BASIC,
                     "kappa decrease factor......................... : %+.4e\n",
                     kappa_decrease_factor_);
...
{
    // Print iteration values
    reporter->printf(R_SOLVER, R_PER_ITERATION, " %6d %+.4e",
                     iteration_counter_,
                     current_iterate_->objectiveSmooth() +
                         current_iterate_->objectiveNonsmooth());
