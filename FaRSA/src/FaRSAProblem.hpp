...
class Problem
{
   public:
     ...
     */
    inline bool numberOfGroups(int& n_g)
    {
        n_g = groups_.size();
        return true;
    };
     ...
     * \return indicator of success (true) or failure (false)
     */
    inline bool numberOfVariables(int& n)
    {
     ...
     */
    virtual bool evaluateObjective(const double* x, double& f) = 0;
    /**
     * Evaluates gradient
     * \param[in] x is a given point/iterate, a constant double array
     * \param[out] g is the gradient value at "x", a double array (return value)
     */
    virtual bool evaluateGradient(const double* x, double* g) = 0;
    /**
     ...
     */
    virtual bool finalizeSolution(const double* x, double f,
                                  const double* g) = 0;
    //@}
