                 ...
                 std::shared_ptr<std::vector<double>> weights, double penalty)
{
    ...
    std::shared_ptr<std::vector<std::vector<int>>> g(new std::vector<std::vector<int>>);
    std::shared_ptr<std::vector<double>>           w(new std::vector<double>);
    groups_ = g;
    weights_ = w;
    // get the actual weights for each group
    number_of_variables_ = 0;
    number_of_groups_ = groups->size();
...
// evaluateGradient
bool GroupL1::evaluateGradient(const Vector& x, const std::vector<int>& indicies, Vector& g)
{
    int group_idx;
    int count = 0;
    for (int i : indicies)
                                           ...
                                           const Vector& v, Vector& Hv)
{
    ...
    for (int i = 0; i < indicies.size(); i++)
    {
        ...
        std::map<int, double>::iterator iter = xgi_vgi_innerproduct.find(group_idx);
        if (iter == xgi_vgi_innerproduct.end())
        {
            ...
            for (int j = start; j <= end; j++)
            {
                inner_prod_Gi = inner_prod_Gi + x.values()[j] * v.values()[count];
                count += 1;
            }
                                            ...
                                            Quantities& quantities, Vector& proxgrad)
{
    ...
    gradient_step.linearCombination(1.0, x, -quantities.stepsizeProximalGradient(), gradfx);
    int group_idx;
    per_group_gradient_step_norm_.clear();
    for (int i = 0; i < gradient_step.length(); i++)
    {
        group_idx = idx_to_group_[i];
        std::map<int, double>::iterator iter = per_group_gradient_step_norm_.find(group_idx);
        if (iter == per_group_gradient_step_norm_.end())
        {
            ...
            for (int j = start; j <= end; j++)
            {
                gradient_step_Gi_norm = gradient_step_Gi_norm + pow(gradient_step.values()[j], 2);
            }
